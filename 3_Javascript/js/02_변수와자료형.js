...
// 즉, 코드블록 내부에서는 선언한 변수(let, const)는
...
// 선언하지 않아서 값이 출력되지 않았음
console.log(foo1); 
console.log(foo2);
// 전역변수이기 때문에 (블록과 관계없음) 접근 가능

// console.log(num1);
// console.log(num2);
// -> 둘 다 블록레벨 스코프이기 때문에 블록 밖에서 접근할 수 없음

// if, for, while, switch, try {} 등 -> 블록 레벨 스코프
...
// var 키워드로 선언된 변수는
// 함수 내 어디서든 접근할 수 있음.
// if, for 등의 일반 블록은 무시하고, 
// var로 선언된 변수가 있는 함수 전체에서 유효함
...
console.log(test2);
// -> test2도 출력 가능.
// -> 왜? 함수레벨 스코프인 var는 일반 블록은 무시하기 때문
...
// 1. var의 호이스팅
// : var로 선언된 변수는 선언만 해당 스코프의 최상단으로 호이스팅 
// 초기화는 원래 코드 위치에서 수행된다.

// 내부적으로 var a; 이렇게 선언만 된 상태
// 에러가 안 나고 undefined가 찍힘.
...
// Java에서 'M' 작성 시 char 타입
// javascript 에서는 string
...
console.log("numbers 배열 값 중 0번쨰 : ", numbers[0],
    typeof numbers[0]
);
console.log("numbers 배열 값 중 0번쨰 : ", numbers[1],
    typeof numbers[1]
);
console.log("numbers 배열 값 중 0번쨰 : ", numbers[2],
    typeof numbers[2]
);
console.log("numbers 배열 값 중 0번쨰 : ", numbers[3],
    typeof numbers[3]
);
